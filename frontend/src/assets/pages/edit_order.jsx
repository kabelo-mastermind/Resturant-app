...
function Edit() {
    ...
    useEffect(() => {
        axios.get(`http://localhost:8001/view_order/${order_id}`)
            .then(res => {
                console.log(res);
                const { name, category, quantity, amount, price } = res.data[0];
                setFormData({ name, category, quantity, amount, price });
            })
    ...
    const handleSubmit = (event) => {
        ...
        axios.put(`http://localhost:8001/edit_order/${order_id}`, values)
            .then(res => {
                console.log(res);
                navigate('/admin');
            })
    ...
    return (
        ...
        <h2 className="text-2xl font-semibold">Update order Item</h2>
        <form onSubmit={handleSubmit} className="mt-4">
            ...
            <div className="mb-4">
                ...
                <label htmlFor="quantity" className="block text-sm font-medium text-gray-700">Amount</label>
                <input
                    type="number"
                    className="w-full px-4 py-2 mt-1 text-sm text-gray-900 border border-gray-300 rounded-md focus:outline-none focus:ring focus:ring-indigo-500 focus:border-indigo-500"
                    id="amount"
                    name="amount"
                    value={values.amount}
                    onChange={e => setFormData({ ...values, amount: e.target.value })}
                />
            </div>
            <div className="mb-4">
